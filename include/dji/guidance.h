...
enum e_sdk_err_code {
  ...
  e_disparity_not_allowed = 4, /**< Disparity or depth image is not allowed to
                                    be selected */
  e_image_frequency_not_allowed = 5, /**< Image frequency must be one of the
                                           enum type e_image_data_frequency */
  e_config_not_ready = 6,            /**< Config is not ready */
  e_online_flag_not_ready = 7,       /**< Online flag is not ready */
  e_stereo_cali_not_ready = 8,       /**< Stereo calibration parameters are not
                                          ready */
...
/**
* @enum  e_vbus_index
* @brief Define logical direction of vbus, i.e. the direction of selected
* Guidance Sensor. Note that they are only defined by the VBUS ports on Guidance
* Core, not by the Guidance Sensors. The comment of each element indicates the
* default direction when Guidance is installed on Matrice 100. However the
* developers can install Guidance in any manner on any device, thus the
* directions might also be different.
*/
enum e_vbus_index {
...
* @brief Define frequency of image data. The supported frequencies are: 5Hz,
* 10Hz, 20Hz. With more images selected, smaller frequency should be selected.
*/
enum e_image_data_frequecy {
...
typedef struct _image_data {
  ...
  char *m_greyscale_image_left[CAMERA_PAIR_NUM];  /**< greyscale image of left
                                                     camera */
  char *m_greyscale_image_right[CAMERA_PAIR_NUM]; /**< greyscale image of right
                                                        camera */
...
*@brief Define ultrasonic data structure. `ultrasonic` is the distance between
*Guidance Sensor and the nearest object detected by ultrasonic sensor. The Unit
*is `mm`. `reliability` is the reliability of this distance measurement, with 1
*meaning reliable and 0 unreliable.
*/
typedef struct _ultrasonic_data {
  unsigned int frame_index; /**< correspondent frame index */
  unsigned int time_stamp;  /**< time stamp of correspondent image captured
                                 in ms */
  short ultrasonic[CAMERA_PAIR_NUM]; /**< distance in mm. -1 means invalid
                                        measurement. */
  unsigned short
      reliability[CAMERA_PAIR_NUM]; /**< reliability of the distance data */
} ultrasonic_data;
...
*@brief Define obstacle distance calculated by fusing vision and ultrasonic
*sensors. Unit is `cm`.
*/
typedef struct _obstacle_distance {
  unsigned int frame_index; /**< correspondent frame index */
  unsigned int
      time_stamp; /**< time stamp of correspondent image captured in ms */
...
/**
* @struct  stereo_cali
* @brief Calibration parameters of cameras. All values will be zero if the
* corresponding sensor is not online.
*/
...
*@brief Parameters of camera exposure. When m_expo_time =
*m_expected_brightness=0, return to default AEC.
* Otherwise set to constant exposure time or constant expected brightness.
*/
typedef struct _exposure_param {
  float m_step;  // adjustment step for auto exposure control (AEC). Default is
                 // 10.
  float m_exposure_time;  // constant exposure time in mini-seconds. Range is
                          // 0.1~20. Default is 7.25.
  unsigned int m_expected_brightness;  // constant expected brightness for AEC.
                                       // Range is 50~200. Default is 85.
...
/**
 *     @fn typedef int (*user_call_back)( int event_type, int data_len, char
 * *data );
 *     @brief Callback function prototype. The developer must write his/her own
 * callback function in this form. In order to achieve best performance, it is
 * suggested not performing any time-consuming processing in the callback
 * function, but only copying the data out. Otherwise the transfer frequency
 * might be slowed down.
 *     @param `event_type` use it to identify the
 * data:image,imu,ultrasonic,velocity or obstacle distance
...
/**
*     @fn int reset_config();
*     @brief  Clear subscribed configure, if you want to subscribe data
...
*     @brief  subscribe velocity data, i.e. velocity of Guidance in body
* coordinate system.
*     @return `error code`. Non-zero if error occurs.
*/
...
*     @brief  subscribe to motion data, i.e. velocity of GUIDANCE in body
* coordinate system.
...
/**
*     @fn int select_greyscale_image( e_vbus_index camera_pair_index, bool
* is_left );
...
*     @brief Set frequecy of image transfer. **Note**: The bandwidth of USB is
* limited. If you subscribe too many images (greyscale image or depth image),
* the frequency should be set relatively small, otherwise the SDK cannot
* guarantee the continuity of image transfer.
...
*     @brief Get the device type. Currently we have two types: Guidance and
* GuidanceLite.
...
*     @brief  Set callback function handler. When data from Guidance comes, it
* will be called by data transfer thread.
...
*	  @param `stereo_cali_pram` Array of calibration parameters for all
*sensors.
...
*	@brief Wait for board ready signal. This function waits 20 seconds for
*Guidance board to get started. If 20
